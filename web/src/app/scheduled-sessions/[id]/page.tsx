...
export default function ScheduledSessionPage() {
  ...
  useEffect(() => {
    ...
    const fetchSession = async () => {
      try {
        ...
        const res = await apiFetch<{ success: boolean; data: { session: SessionData } }>(`/api/sessions/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        
        if (res.success && res.data.session) {
          setSession(res.data.session);
          const r24 = res.data.session.reminders?.find(r => r.reminderType === '24_HOURS');
          const r30 = res.data.session.reminders?.find(r => r.reminderType === '30_MINUTES');
  ...
  const saveReminders = async () => {
    ...
    setSaving(true);
    try {
      const res = await apiFetch<{ success: boolean }>(`/api/schedules/${id}/reminders`, {
        method: 'POST',
        headers: {
  ...
  const initiateReschedule = async () => {
    ...
    try {
      const res = await apiFetch<{ success: boolean; message?: string }>(`/api/schedules/${id}/reschedule`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.success) {
